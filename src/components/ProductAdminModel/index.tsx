...
export default function ProductAdminModel(props: Props) {
    const { product, setProduct, setShowProductModel,  } = props
    ...
    async function handleCreateProduct() {
        ...
        console.log( name, category, description, price, img, slug )
            ...
            <div className="bg-white  w-[100vw] flex flex-col justify-between items-start p-8">
                <div className="flex flex-col gap-2 justify-center items-center">
                    ...
                    <input type="file" className="w-10/12 text-sm"
                        ref={imgInputRef}
                        onChange={async (e) => { setProduct({ ...product, img: await photoConvertProvider(e) }) }} />
                ...
                <div className=" flex justify-around w-full">
                    <button onClick={() => {
                        props.setShowProductModel(false);
                        setProduct({ id: '', name: '', category: props.categories[0].name, img: '', price: '', slug: props.categories[0].slug, description: '' });
                    ...
                    <button
                        ...
                        onClick={() => {console.log(product.id.length>1?'editar':'cria')
                            {
                               product.id.length> 1? handleSaveEdit():handleCreateProduct() 
